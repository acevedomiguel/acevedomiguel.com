...
describe('Color Contrast Compliance', () => {
  ...
  const mockWork = [{
    ...
    highlights: ['Built scalable web applications', 'Mentored junior developers'],
  ...
  describe('Link Accessibility', () => {
    it('provides proper hover states for better accessibility feedback', () => {
      ...
      const linksWithHover = container.querySelectorAll('.hover\\:text-gray-900');
      expect(linksWithHover.length).toBeGreaterThan(0);
      
      // Check that links have underline on hover for better accessibility
      const linksWithUnderline = container.querySelectorAll('.hover\\:underline');
  ...
  describe('Overall Color Compliance', () => {
    it('ensures all components use only WCAG AA compliant color classes', () => {
      ...
      const certs = render(<Certifications certifications={mockCertifications} />);
      ...
      allContainers.forEach(container => {
        ...
        const hasCompliantColors = compliantColors.some(selector => 
