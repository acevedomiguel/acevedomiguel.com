...
export class TechnicalSEO {
   ...
   */
  generateStructuredUrls(pages: Array<{ path: string; title: string }>): Array<{ url: string; slug: string }> {
    ...
    return {
      async headers() {
        return [
          {
            source: '/(.*)',
            headers: Object.entries(this.generateSecurityHeaders()).map(([key, value]) => ({
              key,
              value
            }))
          }
  ...
  validateUrlStructure(url: string): {
    ...
    try {
      ...
      if (urlObj.pathname.includes('_')) {
        recommendations.push('Consider using hyphens instead of underscores in URLs');
      }

      // Check URL length
      if (url.length > 100) {
        recommendations.push('URL is quite long, consider shortening for better usability');
      }
