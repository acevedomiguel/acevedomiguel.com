...
export class StructuredDataGenerator {
	...
	generateWorkExperienceSchema(
		workData: WorkExperienceSchemaData[],
	...
	generateContactPointSchema(
		contactData: ContactPointSchemaData,
	): ContactPointSchema {
	...
	validateStructuredData(schema: any): ValidationResult {
		...
		if (schema["@type"] === "Person") {
			...
			if (!schema.jobTitle)
				errors.push("Person schema missing required jobTitle property");
			if (!schema.email) warnings.push("Person schema missing email property");
		}

		if (schema["@type"] === "WorkExperience") {
			if (!schema.name)
				errors.push("WorkExperience schema missing required name property");
			if (!schema.employer?.name)
				errors.push("WorkExperience schema missing required employer name");
			if (!schema.startDate)
				warnings.push("WorkExperience schema missing startDate");
		}
