...
export class TextTruncator {
	...
	static truncateDescription(
		...
		return TextTruncator.truncateText(text, {
			maxLength,
			ellipsis: "...",
			preserveWords: true,
			minWordLength: 120,
		...
		if (type === "title") {
			...
			if (isTooShort) {
				recommendations.push(
					"Title is too short. Aim for 30-60 characters for better SEO.",
				);
			}
			if (isTooLong) {
				recommendations.push(
					"Title is too long. Keep it under 60 characters to avoid truncation in search results.",
				);
			}
			if (isOptimal) {
			...
			if (isTooShort) {
				recommendations.push(
					"Description is too short. Aim for 120-160 characters for better SEO.",
				);
			}
			if (isTooLong) {
				recommendations.push(
					"Description is too long. Keep it under 160 characters to avoid truncation in search results.",
				);
			}
			if (isOptimal) {
