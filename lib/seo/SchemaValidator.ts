...
export class SchemaValidator {
	...
	private validateSchemaType(
		...
		if (!this.schemaOrgTypes.has(schemaType)) {
			warnings.push(
				`Unknown schema type: ${schemaType}. Verify against Schema.org documentation.`,
			);
		}
	}
	...
	private validateRequiredFields(schema: any, errors: string[]): void {
		...
		required.forEach((field) => {
			if (!this.hasValidValue(schema, field)) {
				errors.push(
					`Missing required field "${field}" for ${schemaType} schema`,
				);
			}
		});
	...
	private validateRecommendedFields(schema: any, warnings: string[]): void {
		...
		recommended.forEach((field) => {
			if (!this.hasValidValue(schema, field)) {
				warnings.push(
					`Missing recommended field "${field}" for ${schemaType} schema`,
				);
			}
		});
	}
			...
			case "BreadcrumbList":
				if (schema.itemListElement && Array.isArray(schema.itemListElement)) {
					...
					if (!this.isSequentialPositions(positions)) {
						errors.push(
							"BreadcrumbList positions must be sequential starting from 1",
						);
					}
				}
				break;
