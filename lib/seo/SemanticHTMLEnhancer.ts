...
export class SemanticHTMLEnhancerImpl implements SemanticHTMLEnhancer {
   ...
   */
  enhanceAccessibility(component: ReactNode, options: AccessibilityOptions = {}): ReactNode {
    const {
  ...
  private renderContentSection(section: ContentSection): any {
    ...
    return {
      ...
      children: [
        ...
        section.content,
        ...(section.children ? section.children.map(child => this.renderContentSection(child)) : [])
      ]
    };
  }
   ...
   */
  private getSemanticElementProps(elementType: SemanticElement): Record<string, any> {
   ...
   */
  private generateDescriptionList(items: Array<{ term: string; description: string }>): any {
    return {
      type: 'dl',
      props: { role: 'list' },
      children: items.map((item, index) => ({
   ...
   */
  createResumeSection(title: string, content: ReactNode, level: 2 | 3 = 2): any {
    ...
    return {
      ...
      children: [
        ...
        job.highlights && job.highlights.length > 0 && {
          ...
          children: job.highlights.map((highlight: string, index: number) => ({
            type: 'li',
            props: { key: index, role: 'listitem' },
            children: highlight
          }))
