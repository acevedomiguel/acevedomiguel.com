...
export class BreadcrumbNavigation {
  ...
  generateBreadcrumbStructuredData(breadcrumbs: BreadcrumbItem[]): string {
    ...
    const absoluteBreadcrumbs = breadcrumbs.map(breadcrumb => ({
      ...breadcrumb,
      url: breadcrumb.url.startsWith('/') ? `${this.baseUrl}${breadcrumb.url}` : breadcrumb.url
    }));
    
    const schema = this.structuredDataGenerator.generateBreadcrumbSchema(absoluteBreadcrumbs);
   ...
   */
  generateBreadcrumbHTML(breadcrumbs: BreadcrumbItem[], separator: string = '>'): string {
   ...
   */
  generateReactBreadcrumbProps(currentPath: string, pageType: string = 'generic') {
