...
export class SEOManager {
  ...
  generatePageSEO(props: SEOManagerProps): SEOResult {
    ...
    const breadcrumbs = props.pageType !== 'home' ? 
      this.generateBreadcrumbs(props.canonicalUrl || '') : undefined;
  ...
  generateStructuredData(props: SEOManagerProps): string {
    ...
    if (props.structuredData) {
      if (props.structuredData.workExperience) {
        const workSchemas = this.structuredDataGenerator.generateWorkExperienceSchema(
      ...
      if (props.structuredData.organization) {
        props.structuredData.organization.forEach(org => {
          const orgSchema = this.structuredDataGenerator.generateOrganizationSchema(org);
          schemas.push(orgSchema);
        });
      }

      if (props.structuredData.contactPoint) {
        const contactSchema = this.structuredDataGenerator.generateContactPointSchema(
      ...
      if (props.structuredData.breadcrumbs) {
        const breadcrumbSchema = this.structuredDataGenerator.generateBreadcrumbSchema(
  ...
  generateBreadcrumbs(path: string): BreadcrumbItem[] {
    const breadcrumbProps = this.breadcrumbNavigation.generateReactBreadcrumbProps(path);
   ...
   */
  private validateSEOData(structuredData: string, metaTags: MetaTag[]): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    try {
      ...
      const schemaValidation = this.schemaValidator.validateMultipleSchemas(schemasArray);
      errors.push(...schemaValidation.errors);
      warnings.push(...schemaValidation.warnings);

      // Validate meta tags
      const requiredMetaTags = ['description', 'og:title', 'og:description', 'twitter:card'];
      requiredMetaTags.forEach(required => {
        const hasTag = metaTags.some(tag => 
          tag.name === required || tag.property === required
        );
        if (!hasTag) {
  ...
  generateResumeSEO(resumeData: any, path: string = '/resume'): SEOResult {
    ...
    const optimizedContent = this.contentOptimizer.organizeResumeContent(resumeData);
    
    // Convert resume data to structured data
    const workExperience = this.structuredDataGenerator.convertResumeToWorkExperience(resumeData);
    const personData = this.structuredDataGenerator.convertResumeToPersonSchema(resumeData);
   ...
   */
  maintainDataConsistency(oldData: any, newData: any): {
    ...
    if (oldData.jobTitle !== newData.jobTitle) {
      recommendations.push('Job title changed - consider updating meta descriptions');
    }

    // Check for new work experience
    if (newData.work && oldData.work) {
      if (newData.work.length > oldData.work.length) {
        recommendations.push('New work experience added - update structured data');
      }
    }
  ...
  generateContentFreshness(lastModified: Date): {
    ...
    const freshness = this.contentOptimizer.generateFreshnessIndicators(lastModified);
  ...
  processExternalLinks(content: string): string {
    ...
    const externalLinkRegex = /<a\s+href="https?:\/\/(?!www\.acevedomiguel\.com)[^"]*"[^>]*>/gi;
  ...
  getSEORecommendations(pageData: PageSEOData): string[] {
    ...
    if (!pageData.title) {
      ...
      recommendations.push('Consider making the title longer (30-60 characters)');
    } else if (pageData.title.length > 60) {
      recommendations.push('Consider shortening the title (under 60 characters)');
    }
    ...
    if (!pageData.description) {
      ...
      recommendations.push('Consider making the description longer (120-160 characters)');
    } else if (pageData.description.length > 160) {
      recommendations.push('Consider shortening the description (under 160 characters)');
    }
    ...
    if (!pageData.keywords || pageData.keywords.length === 0) {
      ...
      recommendations.push('Consider reducing the number of keywords (focus on 5-10 most relevant)');
    }
    ...
    if (pageData.lastModified) {
      ...
      if (freshness.shouldUpdate) {
        recommendations.push('Consider updating content - it has been more than 3 months since last update');
      }
    }
